...
Original file is located at
    https://colab.research.google.com/drive/1DTwME_Crpb-3cmEYtzM2hyqp1ATVp5vh
"""

# ============================================
# Baseline AE (Denoising Autoencoder) — Colab-ready
# - Upload CSV/XLS/XLSX
# - Leave-one-out split (time-aware if timestamp)
# - Train AE with weighted BCE (implicit)
...
# Build CSR user-item matrix for TRAIN (for fast batch construction)
rows, cols = [], []
...
# -------------------------------
# AE Model (Denoising Autoencoder)
# -------------------------------
class AERecommender(nn.Module):
    def __init__(self, n_items, hidden_dims=(600, 200), dropout=0.0):
        super().__init__()
        ...
        for i in range(len(dec_dims)-1):
            # last layer outputs logits (no sigmoid here)
    ...
    def forward(self, x):
        x = self.dropout(x)
        z = self.encoder(x)
        logits = self.decoder(z)  # (B, n_items)
        return logits

model = AERecommender(n_items, hidden_dims=(600, 200), dropout=0.3).to(device)

# Weighted BCE for implicit feedback (alpha > 1 upweights positives)
alpha = 5.0
def weighted_bce_with_logits(logits, target, alpha=1.0):
    # logits, target in shape (B, n_items)
    loss_pos = F.logsigmoid(logits)        # log(sigmoid)
    loss_neg = F.logsigmoid(-logits)       # log(1 - sigmoid)
...
def build_batch_from_csr(user_ids):
    # Create dense batch X (B, n_items) with 0/1 from CSR
    B = len(user_ids)
...
# Denoising: randomly drop some 1s from the input
...
# -------------------------------
# Evaluation helpers (shared style)
# -------------------------------
@torch.no_grad()
def get_user_scores(model, user_id):
    # Build a single-user vector from TRAIN interactions and reconstruct
    x = build_batch_from_csr([user_id]).to(device)
    logits = model(x)[0]  # (n_items,)
    return logits.detach().cpu().numpy()  # raw logits
...
def evaluate(model, train_pos, eval_pos, K_list=(10,20), sample_neg=1000):
    ...
    for u in users:
        ...
        if len(neg_pool) > sample_neg:
            neg_cands = np.random.choice(neg_pool, size=sample_neg, replace=False)
        else:
            neg_cands = neg_pool
...
EPOCHS = 80
BATCH_SIZE = 256
drop_prob = 0.3            # input corruption prob
eval_every = 5
