...
Original file is located at
    https://colab.research.google.com/drive/1XwOiPhtOnq-aTLdhrfVdDM3fksvteMbp
"""

# ============================================
# Baseline BPR-MF (Colab-ready, FIXED)
# - Upload CSV/XLS/XLSX (Dataset1)
...
# implicit binarization for building pairs
...
class BPRMF(nn.Module):
    def __init__(self, n_users, n_items, embed_dim=64):
        ...
        self.I = nn.Embedding(n_items, embed_dim)
        nn.init.xavier_uniform_(self.U.weight); nn.init.xavier_uniform_(self.I.weight)

    def score(self, users, items):
        # FIX: call embeddings; do NOT subscript module
        u_e = self.U(users)    # shape (B, d)
        i_e = self.I(items)    # shape (B, d)
        return (u_e * i_e).sum(dim=1)

    def get_user_item(self):
        return self.U.weight, self.I.weight

model = BPRMF(n_users, n_items, embed_dim=64).to(device)
...
def evaluate(model, train_pos, eval_pos, K_list=(10,20), sample_neg=1000):
    ...
    for u in users:
        ...
        scores = (U[u] * I[candidates]).sum(dim=1).cpu().numpy()
...
def auc_score(model, train_pos, eval_pos, neg_per_pos=50):
    ...
    for u, pos_set in eval_pos.items():
        ...
        pos_s = (U[u] * I[pos_items]).sum(dim=1).cpu().numpy()
        neg_s = (U[u] * I[negs]).sum(dim=1).cpu().numpy()
...
def accuracy_score(model, eval_df, threshold=0.5):
    U, I = model.get_user_item()
    users = eval_df['u'].values; items = eval_df['i'].values
...
print("Start training BPR-MF...")
for epoch in range(1, EPOCHS+1):
    model.train()
    users, pos_items, neg_items = sample_bpr_batch(u_train_pos, n_items, BATCH_SIZE)
    if users.numel()==0:
        print("No training triples available. Check your split/data."); break
...
# ---------- Final Test (table) ----------
print("\nFinal evaluation on TEST for BPRMF Baseline:")
model.eval()
